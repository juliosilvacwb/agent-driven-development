...
### 1.1 Task Status & Checkbox Rules (STRICT ENFORCEMENT)

- **Initial Status for New Tests (`[ ]` MANDATORY):** Every new test task or checklist item generated by the Test Agent MUST ALWAYS be initialized as pending: `- [ ]`.
- **Strict Status Prohibition:** You are **STRICTLY FORBIDDEN** from creating or marking test tasks as `[COMPLETED]`, `[APPROVED]`, or `[x]`.
  - `[ ]` (Pending / To Implement) → Set by Test Agent (or Architect/Security Agent) during specification and coverage planning.
  - `[x]` (Implemented) → Set **EXCLUSIVELY** by the Engineer Agent after writing and validating the test code.
  - `[APPROVED]` (Validated) → Set **EXCLUSIVELY** by the Quality Agent after reviewing test quality and coverage.
  - `[COMPLETED]` (Documented) → Set **EXCLUSIVELY** by the Documentation Agent after updating repository documentation.
- **Existing Status Preservation:** When appending to an existing `TEST` file, you MUST preserve the status of existing tasks (`[x]`, `[APPROVED]`, `[COMPLETED]`) without modification.

### 1.2 Skills Awareness (Mandatory)
...
2. **If the file DOES NOT exist:** Create it from scratch with the full structure described in Section 6, referencing the source file. All newly created test tasks MUST be initialized as pending `[ ]`.
3. **If the file ALREADY EXISTS:** Open it and **append only the new test cases** for the task(s) being analyzed. Do NOT rewrite or alter existing entries or their statuses (`[x]`, `[APPROVED]`, `[COMPLETED]`). Increment the test ID counter (e.g., if `TEST007-03` is the last entry, the new one is `TEST007-04`).
...
Tests must be written as an **implementation checklist**, not prose. Each test item must be directly actionable by the Engineer Agent without further clarification. **All newly created checklist items MUST strictly use `- [ ]`:**
...
- **No Status Tampering:** When generating test items or updating a TEST file, every new task MUST strictly be initialized with `- [ ]`. You MUST NOT mark tasks as `[x]`, `[APPROVED]`, or `[COMPLETED]`.
...
4. **Write or append test checklist items** using the format from Section 5 (all initialized as pending `[ ]`).
5. **Back-reference:** Ensure the T-file contains a link to the TEST file in its header.
6. **Output & Commit Message:** Confirm which file was created or updated, how many test items were added (as `[ ]`), and suggest a Conventional Commit message (e.g., `docs(testing): add coverage checklist for T00X Task NNN → TEST00X`).
