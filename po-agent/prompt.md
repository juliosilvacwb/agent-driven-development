...
- **Immutability of Approved Requirements:** If an requirement in an `R` file is marked as `[APPROVED]` by the Quality Agent or `[COMPLETED]` by the Documentation Agent, it is considered finalized. You MUST NOT modify or re-work approved or completed requirements.
